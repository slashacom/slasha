    ...
    async fn tick(&mut self) -> anyhow::Result<()> {
        ...
        for (container_id, app_id, stats) in &snapshots {
            ...
            let (net_rx_bps, net_tx_bps, disk_read_bps, disk_write_bps) =
                if let Some(p) = self.prev.get(container_id.as_str()) {
        ...
        self.prev.retain(|cid, _| active_ids.contains(cid.as_str()));
...
pub fn compute_cpu_percent(
    ...
    // system_cpu_usage and total_usage are cumulative counters
