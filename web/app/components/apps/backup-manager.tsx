...
function BackupManagerComponent(props: BackupManagerProps) {
  ...
  const body = hasBody ? (
    <>
      {enabled && savedEnabled ? <BackupStatusStrip appSlug={appSlug} /> : null}
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <Input
                ...
                placeholder={secretSet ? '•••••••• (unchanged)' : 'secret key'}
                className="font-mono text-[13px]"
              />
            </div>
          </div>
  ...
  return (
    <>
      <SettingsCard
        icon={DatabaseBackup}
        title="Litestream backups"
        description="Replicate a SQLite DB to object storage for offsite, point-in-time recovery."
