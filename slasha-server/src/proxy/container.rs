...
pub async fn ensure_caddy_volumes(docker: &Docker) -> ProxyResult<()> {
    ...
    for &name in &volumes {
        ...
        docker.create_volume(req).await?;
    }

    Ok(())
}

pub async fn ensure_caddy_running(docker: &Docker) -> ProxyResult<()> {
    pull_caddy_image(docker).await?;

    if !caddy_container_exists(docker).await {
        create_caddy_container(docker).await?;
    }

    start_caddy_container(docker).await;
    wait_for_caddy_ready().await?;

    Ok(())
}

async fn pull_caddy_image(docker: &Docker) -> ProxyResult<()> {
...
async fn caddy_container_exists(docker: &Docker) -> bool {
    docker.inspect_container(CONTAINER_NAME, None).await.is_ok()
}

async fn create_caddy_container(docker: &Docker) -> ProxyResult<()> {
...
async fn start_caddy_container(docker: &Docker) {
    let opts = StartContainerOptionsBuilder::new().build();

    if let Err(e) = docker.start_container(CONTAINER_NAME, Some(opts)).await {
